...
gadget_pair = 1
ch = int(input("Enter your choice for gadget pair, 1 for smash | 2 for clone:  "))
if ch == 1:
    gadget_pair = 1
elif ch == 2:
    gadget_pair = 2
...
# Restart game param
def restart_game():
    global player_1, player_2, ball_x, ball_y, ball_vel_x, ball_vel_y, dummy_ball_x, dummy_ball_y, dummy_ball_vel_x, dummy_ball_vel_y, \
        left_paddle_y, right_paddle_y, second_left_paddle_y, second_right_paddle_y, left_gadget_remaining, right_gadget_remaining, \
...
pygame.display.set_caption("Abe's Ping Pong")
run = True
player_1 = player_2 = 0
direction = [0, 1]
angle = [0, 1, 2]
...
while run:
    ...
    pygame.draw.rect(wn, GREEN, pygame.Rect(right_paddle_x, right_paddle_y, paddle_width, paddle_height))
    ...
    pygame.draw.rect(wn, GREEN, pygame.Rect(second_right_paddle_x, second_right_paddle_y, paddle_width, paddle_height))
    ...
    # endscreen
    winning_font = pygame.font.SysFont('callibri', 50)
    if player_1 >= 3:
        wn.fill(BLACK)
        endscreen = winning_font.render("PLAYER_1 RED WON!!!! Press 'r' for restart", True, WHITE)
        wn.blit(endscreen, (200, 250))

    if player_2 >= 3:
        wn.fill(BLACK)
        endscreen = winning_font.render("PLAYER_2 GREEN WON!!!! Press 'r' for restart ", True, WHITE)
        wn.blit(endscreen, (200, 250))
